...
function buildGridFeatures(
  ...
  return spriteIds.map((id, i) => {
    ...
    const py = row * CELL_HEIGHT_PX + CELL_HEIGHT_PX / 2;
...
export function SdfMapPreview({
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      const features = buildGridFeatures(map, spriteIds, cols);
  ...
  }, [spriteUrl, spriteIds, cols, rows, handleClick, handleMouseMove]);
  ...
  const totalHeight = rows * CELL_HEIGHT_PX;
